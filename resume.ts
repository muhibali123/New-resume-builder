...
document.getElementById("downloadResumeBtn")?.addEventListener("click", () => {
    const resumeData: any = JSON.parse(localStorage.getItem("resumeData") || "{}");
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    if (resumeId) {
        const resumeData: any = JSON.parse(localStorage.getItem(resumeId) || "{}");
...
// Declare jsPDF as a global variable if needed
...
document.getElementById("downloadResumeBtn")?.addEventListener("click", () => {
    ...
    elementHandlers: {
        '#resumeContent': function (element : any, renderer : any) {
